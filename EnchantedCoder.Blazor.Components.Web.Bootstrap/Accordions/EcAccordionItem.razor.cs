...
/// <summary>
/// Single item for <see cref="EcAccordion"/>.
/// </summary>
public partial class EcAccordionItem : ComponentBase
{
	[CascadingParameter] protected EcAccordion ParentAccordion { get; set; }
	...
	/// <summary>
	/// ID of the item (<see cref="EcAccordion.ExpandedItemId"/>). (Gets generated GUID if not set.)
	/// </summary>
	...
	private EcCollapse collapseComponent;

	protected override async Task OnParametersSetAsync()
	{
		await base.OnParametersSetAsync();

		Contract.Requires<InvalidOperationException>(ParentAccordion is not null, "<EcAccordionItem /> has to be placed inside <EcAccordion />.");

		// Issue #182
		// If the accordion items change dynamically, the instances of EcAccordionItem get completely different parameters.
		// EcAccordionItem tracks state (expanded/collapsed) and this state would apply to completely different items after such change.
		...
		// If this turns out to be a problem, we can also consider extracting the whole EcAccordionItem to EcAccordionItemInternal and set the @key for it.
		if ((currentId is not null) && (this.Id != currentId))
		{
			throw new InvalidOperationException("EcAccordionItem.Id cannot be changed. Use @key with same value as Id to help Blazor mapping the right components.");
		}
		else
		{
			currentId = this.Id;
