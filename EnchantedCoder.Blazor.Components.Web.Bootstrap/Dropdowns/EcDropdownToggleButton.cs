...
/// <summary>
/// <see href="https://getbootstrap.com/docs/5.3/components/dropdowns/">Bootstrap Dropdown</see> toggle button which triggers the <see cref="EcDropdownButtonGroup"/> to open.
/// </summary>
public class EcDropdownToggleButton : EcButton, IAsyncDisposable, IEcDropdownToggle
{
	/// <summary>
	/// Application-wide defaults for <see cref="EcDropdownToggleButton"/> and derived components.
	/// </summary>
	public static new ButtonSettings Defaults { get; set; }

	static EcDropdownToggleButton()
	{
		Defaults = EcButton.Defaults with
		{
			Color = ThemeColor.Link
		};
	}

	/// <inheritdoc cref="EcButton.GetDefaults"/>
	...
	[CascadingParameter] protected EcDropdown DropdownContainer { get; set; }
	[CascadingParameter] protected EcNav NavContainer { get; set; }

	[Inject] protected IJSRuntime JSRuntime { get; set; }

	private DotNetObjectReference<EcDropdownToggleButton> dotnetObjectReference;
	private IJSObjectReference jsModule;
	private bool disposed;

	public EcDropdownToggleButton()
	{
	...
	protected override void OnParametersSet()
	{
		...
		if ((DropdownContainer is not null) && (DropdownContainer is not EcDropdownButtonGroup))
		{
			throw new InvalidOperationException($"{nameof(EcDropdownToggleButton)} is expected to used inside {nameof(EcDropdownButtonGroup)} rather than generic {nameof(EcDropdown)} (breaking-change in v2.6.0).");
		}

		if (!String.IsNullOrEmpty(this.Tooltip))
		{
			throw new InvalidOperationException($"{nameof(EcDropdownToggleButton)} does not support {nameof(Tooltip)}.");
		}
	...
	protected override string CoreCssClass =>
		CssClassHelper.Combine(
			...
			((DropdownContainer as EcDropdownButtonGroup)?.Split ?? false) ? "dropdown-toggle-split" : null,
	...
	/// </remarks>
	[JSInvokable("EcDropdown_HandleJsShown")]
	...
	/// </summary>
	[JSInvokable("EcDropdown_HandleJsHidden")]
	...
	{
		jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcDropdown));
	}
