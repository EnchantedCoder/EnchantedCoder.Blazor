...
/// Usually used via <see cref="EcMessageBoxService"/> and <see cref="EcMessageBoxHost"/>.<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcMessageBox">https://EnchantedCoder.blazor.eu/components/EcMessageBox</see>
/// </summary>
public partial class EcMessageBox : ComponentBase
{
	/// <summary>
	...
	/// Default is taken from the underlying <see cref="EcModal"/> (<c>true</c>).
	/// </summary>
	...
	protected ButtonSettings PrimaryButtonSettingsEffective => this.PrimaryButtonSettings ?? GetDefaults().PrimaryButtonSettings ?? throw new InvalidOperationException(nameof(PrimaryButtonSettings) + " default for " + nameof(EcMessageBox) + " has to be set.");

	/// <summary>
	/// Settings for dialog secondary button(s).
	/// </summary>
	[Parameter] public ButtonSettings SecondaryButtonSettings { get; set; }
	protected ButtonSettings SecondaryButtonSettingsEffective => this.SecondaryButtonSettings ?? GetDefaults().SecondaryButtonSettings ?? throw new InvalidOperationException(nameof(SecondaryButtonSettings) + " default for " + nameof(EcMessageBox) + " has to be set.");

	/// <summary>
	/// Settings for underlying <see cref="EcModal"/> component.
	/// </summary>
	[Parameter] public ModalSettings ModalSettings { get; set; }
	protected ModalSettings ModalSettingsEffective => this.ModalSettings ?? GetDefaults().ModalSettings ?? throw new InvalidOperationException(nameof(ModalSettings) + " default for " + nameof(EcMessageBox) + " has to be set.");
	...
	/// <summary>
	/// Additional attributes to be splatted onto an underlying <see cref="EcModal"/>.
	/// </summary>
	[Parameter] public Dictionary<string, object> AdditionalAttributes { get; set; }


	[Inject] protected IStringLocalizer<EcMessageBox> MessageBoxLocalizer { get; set; }

	private EcModal modal;
	...
	{
		result = button;
		await modal.HideAsync();  // fires EcModal.OnClose
	}
