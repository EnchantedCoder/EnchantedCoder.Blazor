...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcModal">https://EnchantedCoder.blazor.eu/components/EcModal</see>
/// </summary>
public partial class EcModal : IAsyncDisposable
{
	/// <summary>
	...
	/// <summary>
	/// Application-wide defaults for the <see cref="EcGrid{TItem}"/>.
	/// </summary>
	...
	protected bool AnimatedEffective => this.Animated ?? this.GetSettings()?.Animated ?? GetDefaults().Animated ?? throw new InvalidOperationException(nameof(Animated) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected ModalSize SizeEffective => this.Size ?? this.GetSettings()?.Size ?? GetDefaults().Size ?? throw new InvalidOperationException(nameof(Size) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected ModalFullscreen FullscreenEffective => this.Fullscreen ?? this.GetSettings()?.Fullscreen ?? GetDefaults().Fullscreen ?? throw new InvalidOperationException(nameof(Fullscreen) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected bool ShowCloseButtonEffective => this.ShowCloseButton ?? this.GetSettings()?.ShowCloseButton ?? GetDefaults().ShowCloseButton ?? throw new InvalidOperationException(nameof(ShowCloseButton) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected bool CloseOnEscapeEffective => this.CloseOnEscape ?? this.GetSettings()?.CloseOnEscape ?? GetDefaults().CloseOnEscape ?? throw new InvalidOperationException(nameof(CloseOnEscape) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected ModalBackdrop BackdropEffective => this.Backdrop ?? this.GetSettings()?.Backdrop ?? GetDefaults().Backdrop ?? throw new InvalidOperationException(nameof(Backdrop) + " default for " + nameof(EcModal) + " has to be set.");

	/// <summary>
	/// Allows scrolling the modal body. Default is <c>false</c>.
	/// </summary>
	[Parameter] public bool? Scrollable { get; set; }
	protected bool ScrollableEffective => this.Scrollable ?? this.GetSettings()?.Scrollable ?? GetDefaults().Scrollable ?? throw new InvalidOperationException(nameof(Scrollable) + " default for " + nameof(EcModal) + " has to be set.");
	...
	protected bool CenteredEffective => this.Centered ?? this.GetSettings()?.Centered ?? GetDefaults().Centered ?? throw new InvalidOperationException(nameof(Centered) + " default for " + nameof(EcModal) + " has to be set.");
	...
	private DotNetObjectReference<EcModal> dotnetObjectReference;
	...
	/// </summary>
	[JSInvokable("EcModal_HandleModalHide")]
	...
	/// </summary>
	[JSInvokable("EcModal_HandleModalHidden")]
	...
	/// </summary>
	[JSInvokable("EcModal_HandleModalShown")]
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		...
		if (shouldOpenModal)
		{
			...
			jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcModal));
			if (disposed)
			{
				return;
			}
	...
	protected virtual async ValueTask DisposeAsyncCore()
	{
		...
		if (jsModule != null)
		{
			// We need to remove backdrop when leaving "page" when EcModal is shown (opened).
			if (opened)
			{
				try
				{
