...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcButton">https://EnchantedCoder.blazor.eu/components/EcButton</see>
/// </summary>
public partial class EcButton : ComponentBase, ICascadeEnabledComponent
{
	/// <summary>
	/// Application-wide defaults for <see cref="EcButton"/> and derived components.
	/// </summary>
	...
	/// <summary>
	/// Position of the icon within the button. Default is <see cref="ButtonIconPlacement.Start" /> (configurable through <see cref="EcButton.Defaults"/>).
	/// </summary>
	[Parameter] public ButtonIconPlacement? IconPlacement { get; set; }
	protected ButtonIconPlacement IconPlacementEffective => this.IconPlacement ?? this.GetSettings()?.IconPlacement ?? GetDefaults()?.IconPlacement ?? throw new InvalidOperationException(nameof(IconPlacement) + " default for " + nameof(EcButton) + " has to be set.");

	/// <summary>
	/// Bootstrap button style - theme color.<br />
	/// Default is taken from <see cref="EcButton.Defaults"/> (<see cref="ThemeColor.None"/> if not customized).
	/// </summary>
	[Parameter] public ThemeColor? Color { get; set; }
	protected ThemeColor ColorEffective => this.Color ?? this.GetSettings()?.Color ?? GetDefaults().Color ?? throw new InvalidOperationException(nameof(Color) + " default for " + nameof(EcButton) + " has to be set.");

	/// <summary>
	/// Button size. Default is <see cref="ButtonSize.Regular"/>.
	/// </summary>
	[Parameter] public ButtonSize? Size { get; set; }
	protected ButtonSize SizeEffective => this.Size ?? this.GetSettings()?.Size ?? GetDefaults().Size ?? throw new InvalidOperationException(nameof(Size) + " default for " + nameof(EcButton) + " has to be set.");

	/// <summary>
	/// <see href="https://getbootstrap.com/docs/5.3/components/buttons/#outline-buttons">Bootstrap "outline" button</see> style.
	/// </summary>
	[Parameter] public bool? Outline { get; set; }
	protected bool OutlineEffective => this.Outline ?? this.GetSettings()?.Outline ?? GetDefaults().Outline ?? throw new InvalidOperationException(nameof(Outline) + " default for " + nameof(EcButton) + " has to be set.");
	...
	/// <summary>
	/// Set state of the embedded <see cref="EcSpinner"/>.
	/// Leave <c>null</c> if you want automated spinner when any of the <see cref="EcButton.OnClick"/> handlers is running.
	...
	private EcTooltip tooltipComponent;

	/// <summary>
	/// Gets basic CSS class(es) which get rendered to every single button. <br/>
	/// Default implementation is <c>"ec-button btn"</c>.
	/// </summary>
	protected virtual string CoreCssClass => "ec-button btn";
	...
	protected string GetColorCssClass()
	{
		if (this.OutlineEffective)
		{
			return this.ColorEffective switch
			{
				...
				_ => throw new InvalidOperationException($"Unknown {nameof(EcButton)} color {this.ColorEffective:g}.")
			};
		}
		return this.ColorEffective switch
		{
	...
	protected string GetSizeCssClass()
	{
		return this.SizeEffective switch
		{
			...
			_ => throw new InvalidOperationException($"Unknown {nameof(EcButton)} {nameof(Size)}: {this.SizeEffective}.")
		};
	}
	...
	{
		// #209 [EcButton] Tooltip does not hide when the button opens EcModal
