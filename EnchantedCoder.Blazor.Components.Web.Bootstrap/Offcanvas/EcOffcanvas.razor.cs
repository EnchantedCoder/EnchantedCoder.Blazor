...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcOffcanvas">https://EnchantedCoder.blazor.eu/components/EcOffcanvas</see>
/// </summary>
public partial class EcOffcanvas : IAsyncDisposable
{
	/// <summary>
	...
	/// <summary>
	/// Application-wide defaults for the <see cref="EcOffcanvas"/> and derived components.
	/// </summary>
	...
	protected OffcanvasPlacement PlacementEffective => this.Placement ?? this.GetSettings()?.Placement ?? GetDefaults().Placement ?? throw new InvalidOperationException(nameof(Placement) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected OffcanvasResponsiveBreakpoint ResponsiveBreakpointEffective => this.ResponsiveBreakpoint ?? this.GetSettings()?.ResponsiveBreakpoint ?? GetDefaults().ResponsiveBreakpoint ?? throw new InvalidOperationException(nameof(ResponsiveBreakpoint) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected OffcanvasSize SizeEffective => this.Size ?? this.GetSettings()?.Size ?? GetDefaults().Size ?? throw new InvalidOperationException(nameof(Size) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected bool ShowCloseButtonEffective => this.ShowCloseButton ?? this.GetSettings()?.ShowCloseButton ?? GetDefaults().ShowCloseButton ?? throw new InvalidOperationException(nameof(ShowCloseButton) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected bool CloseOnEscapeEffective => this.CloseOnEscape ?? this.GetSettings()?.CloseOnEscape ?? GetDefaults().CloseOnEscape ?? throw new InvalidOperationException(nameof(CloseOnEscape) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected OffcanvasBackdrop BackdropEffective => this.Backdrop ?? this.GetSettings()?.Backdrop ?? GetDefaults().Backdrop ?? throw new InvalidOperationException(nameof(Backdrop) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	protected bool ScrollingEnabledEffective => this.ScrollingEnabled ?? this.GetSettings()?.ScrollingEnabled ?? GetDefaults().ScrollingEnabled ?? throw new InvalidOperationException(nameof(ScrollingEnabled) + " default for " + nameof(EcOffcanvas) + " has to be set.");
	...
	private string offcanvasId = "ec" + Guid.NewGuid().ToString("N");
	private DotNetObjectReference<EcOffcanvas> dotnetObjectReference;
	...
	[JSInvokable("EcOffcanvas_HandleOffcanvasHidden")]
	...
	/// </summary>
	[JSInvokable("EcOffcanvas_HandleOffcanvasShown")]
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		...
		if (shouldOpenOffcanvas)
		{
			...
			jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcOffcanvas));
			if (disposed)
			{
				return;
			}
	...
	protected virtual async ValueTask DisposeAsyncCore()
	{
		...
		if (jsModule != null)
		{
			// We need to remove backdrop when leaving "page" when EcOffcanvas is shown (opened).
			if (opened)
			{
				try
				{
	...
	private string GetOffcanvasResponsiveCssClass()
	{
		return this.ResponsiveBreakpointEffective switch
		{
			...
			_ => throw new InvalidOperationException($"Unknown {nameof(EcOffcanvas)}.{nameof(ResponsiveBreakpoint)} value {this.ResponsiveBreakpointEffective:g}.")
		};
	}
	...
	private string GetPlacementCssClass()
	{
		return this.PlacementEffective switch
		{
			...
			_ => throw new InvalidOperationException($"Unknown {nameof(EcOffcanvas)}.{nameof(Placement)} value {this.PlacementEffective:g}.")
		};
	}
	...
	private string GetSizeCssClass()
	{
		return this.SizeEffective switch
		{
			...
			OffcanvasSize.Small => "ec-offcanvas-sm",
			OffcanvasSize.Large => "ec-offcanvas-lg",
			_ => throw new InvalidOperationException($"Unknown {nameof(EcOffcanvas)}.{nameof(Size)} value {this.SizeEffective:g}.")
		};
	}
}