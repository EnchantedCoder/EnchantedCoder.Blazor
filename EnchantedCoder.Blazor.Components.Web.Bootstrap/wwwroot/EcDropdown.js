export function create(element, ecDropdownDotnetObjectReference, reference) {
	if (!element) {
		return;
	}
	element.ecDropdownDotnetObjectReference = ecDropdownDotnetObjectReference;
...
function handleDropdownShown(event) {
	event.target.ecDropdownDotnetObjectReference.invokeMethodAsync('EcDropdown_HandleJsShown');
};

function handleDropdownHidden(event) {
	event.target.ecDropdownDotnetObjectReference.invokeMethodAsync('EcDropdown_HandleJsHidden');
};
...
export function dispose(element) {
	...
	element.ecDropdownDotnetObjectReference = null;
	var d = bootstrap.Dropdown.getInstance(element);
	if (d) {
		d.dispose();
	}
