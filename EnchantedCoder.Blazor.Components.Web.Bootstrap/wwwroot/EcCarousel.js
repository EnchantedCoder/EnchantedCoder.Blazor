export function initialize(element, ecCarouselDotnetObjectReference, options) {
	if (!element) {
		return;
	}
	var carousel = new bootstrap.Carousel(element, options);

	element.ecCarouselDotnetObjectReference = ecCarouselDotnetObjectReference;
...
function handleSlide(event) {
	event.target.ecCarouselDotnetObjectReference.invokeMethodAsync('EcCarousel_HandleSlide', event.from, event.to, event.direction);
}

function handleSlid(event) {
	event.target.ecCarouselDotnetObjectReference.invokeMethodAsync('EcCarousel_HandleSlid', event.from, event.to, event.direction);
}
...
export function dispose(element) {
	...
	element.ecCarouselDotnetObjectReference = null;
