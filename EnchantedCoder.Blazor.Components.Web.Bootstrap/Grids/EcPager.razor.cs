...
/// <summary>
/// Pager.<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcPager">https://EnchantedCoder.blazor.eu/components/EcPager</see>
/// </summary>
public partial class EcPager : ComponentBase
{
	/// <summary>
	/// Application-wide defaults for <see cref="EcPager"/>.
	/// </summary>
	...
	protected IconBase FirstPageIconEffective => this.FirstPageIcon ?? this.GetSettings()?.FirstPageIcon ?? GetDefaults().FirstPageIcon ?? throw new InvalidOperationException(nameof(FirstPageIcon) + " default for " + nameof(EcPager) + " has to be set.");

	/// <summary>
	/// Icon for the "Last page" button.
	/// </summary>
	[Parameter] public IconBase LastPageIcon { get; set; }
	protected IconBase LastPageIconEffective => this.LastPageIcon ?? this.GetSettings()?.LastPageIcon ?? GetDefaults().LastPageIcon ?? throw new InvalidOperationException(nameof(LastPageIcon) + " default for " + nameof(EcPager) + " has to be set.");

	/// <summary>
	/// Icon for the "Previous page" button.
	/// </summary>
	[Parameter] public IconBase PreviousPageIcon { get; set; }
	protected IconBase PreviousPageIconEffective => this.PreviousPageIcon ?? this.GetSettings()?.PreviousPageIcon ?? GetDefaults().PreviousPageIcon ?? throw new InvalidOperationException(nameof(PreviousPageIcon) + " default for " + nameof(EcPager) + " has to be set.");

	/// <summary>
	/// Icon for the "Next page" button.
	/// </summary>
	[Parameter] public IconBase NextPageIcon { get; set; }
	protected IconBase NextPageIconEffective => this.NextPageIcon ?? this.GetSettings()?.NextPageIcon ?? GetDefaults().NextPageIcon ?? throw new InvalidOperationException(nameof(NextPageIcon) + " default for " + nameof(EcPager) + " has to be set.");

	/// <summary>
	/// Count of numbers to display. Default value is 10.
	/// </summary>
	[Parameter] public int? NumericButtonsCount { get; set; }
	protected int NumericButtonsCountEffective => this.NumericButtonsCount ?? this.GetSettings()?.NumericButtonsCount ?? GetDefaults().NumericButtonsCount ?? throw new InvalidOperationException(nameof(NumericButtonsCount) + " default for " + nameof(EcPager) + " has to be set.");
