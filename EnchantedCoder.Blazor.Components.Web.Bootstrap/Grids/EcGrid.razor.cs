...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcGrid">https://EnchantedCoder.blazor.eu/components/EcGrid</see>
/// </summary>
/// <typeparam name="TItem">Type of row data item.</typeparam>
[CascadingTypeParameter(nameof(TItem))]
public partial class EcGrid<TItem> : ComponentBase, IDisposable
{
	/// <summary>
	...
	/// <summary>
	/// Set of settings to be applied to the component instance (overrides <see cref="EcGrid.Defaults"/>, overridden by individual parameters).
	/// </summary>
	...
	protected GridContentNavigationMode ContentNavigationModeEffective => this.ContentNavigationMode ?? this.GetSettings()?.ContentNavigationMode ?? GetDefaults().ContentNavigationMode ?? throw new InvalidOperationException(nameof(ContentNavigationMode) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected int PageSizeEffective => this.PageSize ?? this.GetSettings()?.PageSize ?? GetDefaults().PageSize ?? throw new InvalidOperationException(nameof(PageSize) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected bool ShowFooterWhenEmptyDataEffective => this.ShowFooterWhenEmptyData ?? this.GetSettings()?.ShowFooterWhenEmptyData ?? GetDefaults().ShowFooterWhenEmptyData ?? throw new InvalidOperationException(nameof(ShowFooterWhenEmptyData) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected ButtonSettings LoadMoreButtonSettingsEffective => this.LoadMoreButtonSettings ?? this.GetSettings()?.LoadMoreButtonSettings ?? GetDefaults().LoadMoreButtonSettings ?? throw new InvalidOperationException(nameof(LoadMoreButtonSettings) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	/// <summary>
	/// Custom CSS class to render with <c>div</c> element wrapping the main <c>table</c> (<see cref="EcPager"/> is not wrapped in this <c>div</c> element).
	/// </summary>
	...
	protected float ItemRowHeightEffective => this.ItemRowHeight ?? this.GetSettings()?.ItemRowHeight ?? GetDefaults().ItemRowHeight ?? throw new InvalidOperationException(nameof(ItemRowHeight) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected int PlaceholdersRowCountEffective => this.PlaceholdersRowCount ?? this.GetSettings()?.PlaceholdersRowCount ?? GetDefaults().PlaceholdersRowCount ?? throw new InvalidOperationException(nameof(PlaceholdersRowCount) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected int OverscanCountEffective => this.OverscanCount ?? this.GetSettings()?.OverscanCount ?? GetDefaults().OverscanCount ?? throw new InvalidOperationException(nameof(OverscanCount) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected bool ResponsiveEffective => this.Responsive ?? this.GetSettings()?.Responsive ?? GetDefaults().Responsive ?? throw new InvalidOperationException(nameof(Responsive) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	protected bool StripedEffective => this.Striped ?? this.GetSettings()?.Striped ?? GetDefaults().Striped ?? throw new InvalidOperationException(nameof(Striped) + " default for " + nameof(EcGrid) + " has to be set.");
	...
	/// </summary>
	protected virtual GridSettings GetDefaults() => EcGrid.Defaults;

	[Inject] protected IStringLocalizer<EcGrid> EcGridLocalizer { get; set; }

	private List<IEcGridColumn<TItem>> columnsList;
	private HashSet<string> columnIds;

	private CollectionRegistration<IEcGridColumn<TItem>> columnsListRegistration;
	...
	public EcGrid()
	{
		columnsList = new List<IEcGridColumn<TItem>>();
		columnsListRegistration = new CollectionRegistration<IEcGridColumn<TItem>>(columnsList, async () => await InvokeAsync(this.StateHasChanged), () => isDisposed, HandleColumnAdded, HandleColumnRemoved);
	}
	...
	/// </summary>
	protected List<IEcGridColumn<TItem>> GetColumnsToRender()
	{
	...
	protected virtual string GetTableElementCssClass(bool rendersData)
	{
		...
		return CssClassHelper.Combine("ec-grid table",
	...
	private async Task HandleSortingClick(IEcGridColumn<TItem> newSortColumn)
	{
	...
	private void HandleColumnAdded(IEcGridColumn<TItem> column)
	{
	...
	private void HandleColumnRemoved(IEcGridColumn<TItem> column)
	{
