...
/// <summary>
/// Basic calendar building block. Used for <see cref="EcInputDate{TValue}"/> and <see cref="EcInputDateRange"/> implementation.<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcCalendar">https://EnchantedCoder.blazor.eu/components/EcCalendar</see>
/// </summary>
public partial class EcCalendar
{
	/// <summary>
	...
	/// <summary>
	/// Application-wide defaults for the <see cref="EcCalendar"/>.
	/// </summary>
	...
	/// Default is <c>1.1.1900</c> (configurable from <see cref="EcCalendar.Defaults"/>).
	/// </summary>
	[Parameter] public DateTime? MinDate { get; set; }
	protected DateTime MinDateEffective => this.MinDate ?? this.GetSettings()?.MinDate ?? GetDefaults().MinDate ?? throw new InvalidOperationException(nameof(MinDate) + " default for " + nameof(EcCalendar) + " has to be set.");

	/// <summary>
	/// Last date selectable from the calendar.<br />
	/// Default is <c>31.12.2099</c> (configurable from <see cref="EcCalendar.Defaults"/>).
	/// </summary>
	[Parameter] public DateTime? MaxDate { get; set; }
	protected DateTime MaxDateEffective => this.MaxDate ?? this.GetSettings()?.MaxDate ?? this.GetDefaults().MaxDate ?? throw new InvalidOperationException(nameof(MaxDate) + " default for " + nameof(EcCalendar) + " has to be set.");
		...
		for (var week = 0; week < 6; week++)
		{
			...
			for (int day = 0; day < 7; day++)
			{
				...
				string cssClass = CssClassHelper.Combine(
					...
					(currentDay == today) ? "ec-calendar-today" : null,
