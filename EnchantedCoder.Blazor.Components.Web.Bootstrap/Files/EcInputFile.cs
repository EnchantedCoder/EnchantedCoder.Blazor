...
using EnchantedCoder.Blazor.Components.Web.Infrastructure;
using Microsoft.AspNetCore.Components.Forms;

namespace EnchantedCoder.Blazor.Components.Web.Bootstrap;

/// <summary>
/// Wraps <see cref="EcInputFileCore"/> as Bootstrap form control (incl. <c>Label</c> etc.)<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcInputFile">https://EnchantedCoder.blazor.eu/components/EcInputFile</see>
/// </summary>
public partial class EcInputFile : ComponentBase, ICascadeEnabledComponent, IFormValueComponent, IInputWithSize
{
	/// <summary>
	/// Application-wide defaults for the <see cref="EcInputFile"/> and derived components.
	/// </summary>
	...
	/// <summary>
	/// Maximum number of concurrent uploads. Default is <c>6</c> (from <see cref="EcInputFileCore"/>).
	/// </summary>
	...
	protected InputSize InputSizeEffective => this.InputSize ?? GetSettings()?.InputSize ?? GetDefaults()?.InputSize ?? throw new InvalidOperationException(nameof(InputSize) + " default for " + nameof(EcInputFile) + " has to be set.");
	...
	/// </summary>
	public int FileCount => ecInputFileCoreComponentReference.FileCount;

	/// <summary>
	/// ID if the input element. Autogenerated when used with label.
	/// </summary>
	protected string InputId { get; private set; } = "ec" + Guid.NewGuid().ToString("N");
	...
	private protected EcInputFileCore ecInputFileCoreComponentReference;

	/// <summary>
	/// Gets list of files chosen.
	/// </summary>
	public Task<FileInfo[]> GetFilesAsync() => ecInputFileCoreComponentReference.GetFilesAsync();

	/// <summary>
	/// Clears associated input-file element and resets component to initial state.
	/// </summary>
	public Task ResetAsync() => ecInputFileCoreComponentReference.ResetAsync();
	...
	/// <remarks>
	/// We do not want to make the EnchantedCoder.Blazor library dependent on WebAssembly libraries (IAccessTokenProvider and such). Therefor the accessToken here...
	/// </remarks>
	public Task StartUploadAsync(string accessToken = null) => ecInputFileCoreComponentReference?.StartUploadAsync(accessToken);

	/// <summary>
	/// Uploads the file(s).
	/// </summary>
	/// <param name="accessToken">Authorization Bearer Token to be used for upload (i.e. use IAccessTokenProvider).</param>
	public Task<UploadCompletedEventArgs> UploadAsync(string accessToken = null) => ecInputFileCoreComponentReference?.UploadAsync(accessToken);
	...
	protected override void BuildRenderTree(RenderTreeBuilder builder)
	{
		...
		EcFormValueRenderer.Current.Render(1, builder, this);
	}

	void IFormValueComponent.RenderValue(RenderTreeBuilder builder)
	{
		builder.OpenComponent<EcInputFileCore>(1);
		builder.AddAttribute(1001, nameof(EcInputFileCore.Id), this.InputId);
		builder.AddAttribute(1002, nameof(EcInputFileCore.UploadUrl), this.UploadUrl);
		builder.AddAttribute(1003, nameof(EcInputFileCore.Multiple), this.Multiple);
		builder.AddAttribute(1004, nameof(EcInputFileCore.OnChange), EventCallback.Factory.Create<InputFileChangeEventArgs>(this, InvokeOnChangeAsync));
		builder.AddAttribute(1005, nameof(EcInputFileCore.OnProgress), EventCallback.Factory.Create<UploadProgressEventArgs>(this, InvokeOnProgressAsync));
		builder.AddAttribute(1006, nameof(EcInputFileCore.OnFileUploaded), EventCallback.Factory.Create<FileUploadedEventArgs>(this, InvokeOnFileUploadedAsync));
		builder.AddAttribute(1007, nameof(EcInputFileCore.OnUploadCompleted), EventCallback.Factory.Create<UploadCompletedEventArgs>(this, InvokeOnUploadCompletedAsync));
		builder.AddAttribute(1008, nameof(EcInputFileCore.Accept), this.Accept);
		builder.AddAttribute(1009, nameof(EcInputFileCore.MaxFileSize), this.MaxFileSize);
		builder.AddAttribute(1010, nameof(EcInputFileCore.MaxParallelUploads), this.MaxParallelUploads);
		...
		builder.AddAttribute(1012, nameof(EcInputFileCore.Enabled), CascadeEnabledComponent.EnabledEffective(this));
		builder.AddComponentReferenceCapture(1013, r => ecInputFileCoreComponentReference = (EcInputFileCore)r);
