...
public partial class EcAutosuggestInternal<TItem, TValue> : IAsyncDisposable
{
	...
	private EcAutosuggestInputInternal autosuggestInput;
	private TValue lastKnownValue;
	private bool dataProviderInProgress;
	private DotNetObjectReference<EcAutosuggestInternal<TItem, TValue>> dotnetObjectReference;

	internal string ChipValue => userInput;

	public EcAutosuggestInternal()
	{
	...
	[JSInvokable("EcAutosuggestInternal_HandleInputKeyDown")]
	...
	{
		jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcAutosuggest));
	}

	[JSInvokable("EcAutosuggestInternal_HandleDropdownHidden")]
