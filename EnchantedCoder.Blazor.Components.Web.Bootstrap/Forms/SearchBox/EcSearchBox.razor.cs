...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcSearchBox">https://EnchantedCoder.blazor.eu/components/EcSearchBox</see>
/// </summary>
/// <typeparam name="TItem"></typeparam>
public partial class EcSearchBox<TItem> : IAsyncDisposable
{
	/// <summary>
	/// Returns application-wide defaults for the component.
	/// Enables overriding defaults in descendants (use separate set of defaults).
	/// </summary>
	protected virtual SearchBoxSettings GetDefaults() => EcSearchBox.Defaults;

	/// <summary>
	/// Set of settings to be applied to the component instance (overrides <see cref="EcSearchBox.Defaults"/>, overridden by individual parameters).
	/// </summary>
	...
	protected InputSize InputSizeEffective => this.InputSize ?? this.GetSettings()?.InputSize ?? this.GetDefaults()?.InputSize ?? throw new InvalidOperationException(nameof(InputSize) + " default for " + nameof(EcSearchBox) + " has to be set.");

	/// <summary>
	/// Minimum length to call the data provider (display any results). Default is <c>2</c>.
	/// </summary>
	[Parameter] public int? MinimumLength { get; set; }
	protected int MinimumLengthEffective => this.MinimumLength ?? this.GetSettings()?.MinimumLength ?? GetDefaults().MinimumLength ?? throw new InvalidOperationException(nameof(MinimumLength) + " default for " + nameof(EcSearchBox) + " has to be set.");

	/// <summary>
	/// Debounce delay in milliseconds. Default is <c>300</c> ms.
	/// </summary>
	[Parameter] public int? Delay { get; set; }
	protected int DelayEffective => this.Delay ?? this.GetSettings()?.Delay ?? GetDefaults().Delay ?? throw new InvalidOperationException(nameof(Delay) + " default for " + nameof(EcSearchBox) + " has to be set.");
	...
	private string dropdownToggleElementId = "ec" + Guid.NewGuid().ToString("N");
	private string dropdownId = "ec" + Guid.NewGuid().ToString("N");
	private string inputId = "ec" + Guid.NewGuid().ToString("N");
	private List<TItem> searchResults = new();
	private EcDropdownToggleElement dropdownToggle;
	...
	private DotNetObjectReference<EcSearchBox<TItem>> dotnetObjectReference;
	...
	{
		jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcSearchBox));
	}
	...
	[JSInvokable("EcSearchBox_HandleInputKeyDown")]
