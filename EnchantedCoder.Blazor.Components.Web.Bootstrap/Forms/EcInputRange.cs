...
public class EcInputRange<TValue> : EcInputBase<TValue>
{
	...
	/// <summary>
	/// Returns <see cref="EcInputRange"/> defaults.
	/// Enables to not share defaults in descendants with base classes.
	/// Enables to have multiple descendants which differs in the default values.
	/// </summary>
	protected override InputRangeSettings GetDefaults() => EcInputRange.Defaults;

	/// <summary>
	/// Set of settings to be applied to the component instance (overrides <see cref="EcInputRange.Defaults"/>, overridden by individual parameters).
	/// </summary>
	...
	/// <summary>
	/// By default, <code>EcInputRange</code> snaps to integer values. To change this, you can specify a step value.
	/// </summary>
	...
	protected virtual BindEvent BindEventEffective => BindEvent ?? GetSettings()?.BindEvent ?? GetDefaults()?.BindEvent ?? throw new InvalidOperationException(nameof(BindEvent) + " default for " + nameof(EcInputRange<TValue>) + " has to be set.");
	...
	{
		throw new InvalidOperationException("EcInputRange displays no text value and receives the initial value as float, therefore, this method must not be called.");
	}
}
