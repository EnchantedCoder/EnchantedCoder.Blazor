...
/// </summary>
public abstract class EcInputBase<TValue> : InputBase<TValue>, ICascadeEnabledComponent, IFormValueComponent
{
	/// <summary>
	...
	/// <summary>
	/// Return <see cref="EcInputBase{TValue}"/> defaults.
	/// Enables to not share defaults in descendants with base classes.
	/// Enables to have multiple descendants which differs in the default values.
	/// </summary>
	protected virtual InputSettings GetDefaults() => EcInputBase.Defaults;

	/// <summary>
	/// Set of settings to be applied to the component instance (overrides <see cref="EcInputBase.Defaults"/>, overridden by individual parameters).
	/// </summary>
	/// <remarks>
	...
	/// Default is <see cref="ValidationMessageMode.Regular"/>, you can override application-wide default for all inputs in <see cref="EcInputBase.Defaults"/>.
	/// </summary>
	[Parameter] public ValidationMessageMode? ValidationMessageMode { get; set; }
	protected ValidationMessageMode ValidationMessageModeEffective => this.ValidationMessageMode ?? this.GetSettings()?.ValidationMessageMode ?? GetDefaults().ValidationMessageMode ?? EcInputBase.Defaults?.ValidationMessageMode ?? throw new InvalidOperationException(nameof(ValidationMessageMode) + " default for " + nameof(EcInputBase<TValue>) + " has to be set.");
	...
	/// <summary>
	/// When <c>true</c>, <see cref="EcChipGenerator"/> is used to generate chip item(s). Default is <c>true</c>.
	/// </summary>
	...
	/// </summary>
	private protected virtual string CoreCssClass => CssClassHelper.Combine("ec-form-group position-relative",
	...
	protected override void OnParametersSet()
	{
		...
		if ((this is IInputWithLabelType inputWithLabelType)
			...
			&& (inputWithLabelType.LabelType == EnchantedCoder.Blazor.Components.Web.Bootstrap.LabelType.Floating)
	...
	/// <summary>
	/// Renders validation message (component <see cref="EcValidationMessage{TValue}" />) when not disabled (<seealso cref="ValidationMessageModeEffective" />).
	/// </summary>
	protected virtual void BuildRenderValidationMessage(RenderTreeBuilder builder)
	{
		if (this.ValidationMessageModeEffective != Bootstrap.ValidationMessageMode.None) // if: performance
		{
			...
			//</div>
			builder.OpenComponent<EcValidationMessage<TValue>>(1);
			if (autoCreatedEditContext != null)
			{
				builder.AddAttribute(2, nameof(EcValidationMessage<TValue>.EditContext), autoCreatedEditContext);
			}
			builder.AddAttribute(3, nameof(EcValidationMessage<TValue>.For), ValueExpression);
			builder.AddAttribute(4, nameof(EcValidationMessage<TValue>.Mode), ValidationMessageModeEffective);
			builder.CloseComponent();
		}
	}
	...
	{
		builder.OpenComponent<EcChipGenerator>(0);
		builder.AddAttribute(1, nameof(EcChipGenerator.ChildContent), (RenderFragment)RenderChipTemplate);
		builder.AddAttribute(2, nameof(EcChipGenerator.ChipRemoveAction), GetChipRemoveAction());
	...
	protected void RenderChipTemplate(RenderTreeBuilder builder)
	{
		...
		else
		{
			...
			builder.AddAttribute(1, "class", "ec-chip-list-label");
			builder.OpenRegion(2);
