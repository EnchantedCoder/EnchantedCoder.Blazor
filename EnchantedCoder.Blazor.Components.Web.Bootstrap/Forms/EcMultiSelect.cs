...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcMultiSelect">https://EnchantedCoder.blazor.eu/components/EcMultiSelect</see>
/// </summary>
/// <typeparam name="TValue">Type of values.</typeparam>
/// <typeparam name="TItem">Type of items.</typeparam>
public class EcMultiSelect<TValue, TItem> : EcInputBase<List<TValue>>, IInputWithSize
{
	/// <summary>
	/// Return <see cref="EcMultiSelect{TValue, TItem}"/> defaults.
	/// Enables to not share defaults in descendants with base classes.
	/// Enables to have multiple descendants which differs in the default values.
	/// </summary>
	protected override MultiSelectSettings GetDefaults() => EcMultiSelect.Defaults;

	/// <summary>
	/// Set of settings to be applied to the component instance (overrides <see cref="EcMultiSelect.Defaults"/>, overridden by individual parameters).
	/// </summary>
	...
	protected InputSize InputSizeEffective => this.InputSize ?? GetSettings()?.InputSize ?? GetDefaults()?.InputSize ?? throw new InvalidOperationException(nameof(InputSize) + " default for " + nameof(EcMultiSelect) + " has to be set.");
	...
	private protected override string CoreInputCssClass => "ec-multi-select-input form-select user-select-none";
	...
	private EcMultiSelectInternal<TValue, TItem> ecMultiSelectInternalComponent;
	...
	/// <inheritdoc cref="EcInputBase{TValue}.FocusAsync"/>
	public override async ValueTask FocusAsync()
	{
		if (ecMultiSelectInternalComponent == null)
		{
			throw new InvalidOperationException($"Cannot focus {this.GetType()}. The method must be called after first render.");
		}

		await ecMultiSelectInternalComponent.FocusAsync();
	}
	...
	protected override void BuildRenderInput(RenderTreeBuilder builder)
	{
		...
		builder.OpenComponent<EcMultiSelectInternal<TValue, TItem>>(100);
		builder.AddAttribute(101, nameof(EcMultiSelectInternal<TValue, TItem>.InputId), InputId);
		builder.AddAttribute(102, nameof(EcMultiSelectInternal<TValue, TItem>.InputCssClass), GetInputCssClassToRender());
		builder.AddAttribute(103, nameof(EcMultiSelectInternal<TValue, TItem>.InputText), GetInputText());
		builder.AddAttribute(104, nameof(EcMultiSelectInternal<TValue, TItem>.EnabledEffective), EnabledEffective);
		builder.AddAttribute(105, nameof(EcMultiSelectInternal<TValue, TItem>.ItemsToRender), itemsToRender);
		builder.AddAttribute(106, nameof(EcMultiSelectInternal<TValue, TItem>.TextSelector), TextSelector);
		builder.AddAttribute(107, nameof(EcMultiSelectInternal<TValue, TItem>.ValueSelector), ValueSelector);
		builder.AddAttribute(108, nameof(EcMultiSelectInternal<TValue, TItem>.Value), Value);
		builder.AddAttribute(109, nameof(EcMultiSelectInternal<TValue, TItem>.NullDataText), NullDataText);
		builder.AddAttribute(110, nameof(EcMultiSelectInternal<TValue, TItem>.ItemSelectionChanged), EventCallback.Factory.Create<EcMultiSelectInternal<TValue, TItem>.SelectionChangedArgs>(this, args => HandleItemSelectionChanged(args.Checked, args.Item)));
		builder.AddAttribute(111, nameof(EcMultiSelectInternal<TValue, TItem>.InputGroupStartText), InputGroupStartText);
		builder.AddAttribute(112, nameof(EcMultiSelectInternal<TValue, TItem>.InputGroupStartTemplate), InputGroupStartTemplate);
		builder.AddAttribute(113, nameof(EcMultiSelectInternal<TValue, TItem>.InputGroupEndText), InputGroupEndText);
		builder.AddAttribute(114, nameof(EcMultiSelectInternal<TValue, TItem>.InputGroupEndTemplate), InputGroupEndTemplate);

		builder.AddMultipleAttributes(200, this.AdditionalAttributes);

		builder.AddComponentReferenceCapture(300, r => ecMultiSelectInternalComponent = (EcMultiSelectInternal<TValue, TItem>)r);
