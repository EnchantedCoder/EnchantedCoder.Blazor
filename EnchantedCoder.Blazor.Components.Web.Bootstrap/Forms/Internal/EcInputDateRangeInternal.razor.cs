...
public partial class EcInputDateRangeInternal : InputBase<DateTimeRange>, IAsyncDisposable, IInputWithSize
{
	...
	private EcDropdownToggleElement fromDropdownToggleElement;
	private EcDropdownToggleElement toDropdownToggleElement;
	...
	protected void HandleFromChanged(ChangeEventArgs changeEventArgs)
	{
		...
		if (EcInputDate<DateTime>.TryParseDateTimeOffsetFromString((string)changeEventArgs.Value, null, out var fromDate))
		{
	...
	protected void HandleToChanged(ChangeEventArgs changeEventArgs)
	{
		...
		if (EcInputDate<DateTime>.TryParseDateTimeOffsetFromString((string)changeEventArgs.Value, null, out var toDate))
		{
	...
	private async Task OpenDropDownAsync(EcDropdownToggleElement triggerElement)
	{
		Contract.Assert<ArgumentNullException>(triggerElement != null, nameof(triggerElement));
		await triggerElement.ShowAsync();
	}

	private async Task CloseDropdownAsync(EcDropdownToggleElement triggerElement)
	{
		Contract.Assert<ArgumentNullException>(triggerElement != null, nameof(triggerElement));
	...
	protected async Task HandleDateRangeClick(DateTimeRange value, EcDropdownToggleElement dropdownElement)
	{
