...
public partial class EcInputDateInternal<TValue> : InputBase<TValue>, IAsyncDisposable, IInputWithSize
{
	...
	private EcDropdownToggleElement ecDropdownToggleElement;
	...
	protected override string FormatValueAsString(TValue value) => EcInputDate<TValue>.FormatValue(value);
	...
	private void HandleValueChanged(ChangeEventArgs changeEventArgs)
	{
		...
		if (EcInputDate<DateTime>.TryParseDateTimeOffsetFromString((string)changeEventArgs.Value, null, out var date))
		{
			parsingFailed = false;
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		...
		if (RenderIcon)
		{
			jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorBootstrapModuleAsync(nameof(EcInputDate));
			await jsModule.InvokeVoidAsync("addOpenAndCloseEventListeners", ecDropdownToggleElement.ElementReference, (this.CalendarIconEffective is not null) ? iconWrapperElement : null);
		}
	}
	...
	{
		Contract.Requires<InvalidOperationException>(ecDropdownToggleElement != null);
		await ecDropdownToggleElement.HideAsync();
	}
	...
	protected virtual async ValueTask DisposeAsyncCore()
	{
		...
		try
		{
			if (ecDropdownToggleElement is not null)
			{
				await CloseDropdownAsync();
			}
