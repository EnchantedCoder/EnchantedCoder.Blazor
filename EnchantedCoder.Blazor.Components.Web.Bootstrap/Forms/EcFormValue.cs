...
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcFormValue">https://EnchantedCoder.blazor.eu/components/EcFormValue</see>
/// </summary>
public class EcFormValue : ComponentBase, IFormValueComponent, IFormValueComponentWithInputGroups, IInputWithSize
{
	/// <summary>
	/// Application-wide defaults for <see cref="EcFormValue"/> and derived components.
	/// </summary>
	...
	protected InputSize InputSizeEffective => this.InputSize ?? GetSettings()?.InputSize ?? GetDefaults()?.InputSize ?? throw new InvalidOperationException(nameof(InputSize) + " default for " + nameof(EcFormValue) + " has to be set.");
	...
	public void RenderValue(RenderTreeBuilder builder)
	{
		...
		{
			// workaround for [EcFormValue] Shrunk when displaying null/empty value #208
