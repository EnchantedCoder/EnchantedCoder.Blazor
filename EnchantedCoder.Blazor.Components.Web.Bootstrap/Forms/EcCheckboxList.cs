...
/// <summary>
/// Renders a multi-selection list of <see cref="EcCheckbox"/> controls.<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcCheckboxList">https://EnchantedCoder.blazor.eu/components/EcCheckboxList</see>
/// </summary>
public class EcCheckboxList<TValue, TItem> : EcInputBase<List<TValue>> // cannot use an array: https://github.com/dotnet/aspnetcore/issues/15014
{
	/// <summary>
	/// Items to display.
	/// </summary>
	...
	/// <summary>
	/// Additional CSS class(es) for underlying <see cref="EcCheckbox" />.
	/// </summary>
	[Parameter] public string ItemCssClass { get; set; }

	/// <summary>
	/// Additional CSS class(es) for the <see cref="EcCheckbox" />.
	/// </summary>
	[Parameter] public Func<TItem, string> ItemCssClassSelector { get; set; }

	/// <summary>
	/// Additional CSS class(es) for the input element of the <see cref="EcCheckbox" />.
	/// </summary>
	...
	/// <summary>
	/// Additional CSS class(es) for the text of the <see cref="EcCheckbox" />.
	/// </summary>
	...
	protected override void BuildRenderInput(RenderTreeBuilder builder)
	{
		...
		if (itemsToRender.Count > 0)
		{
			...
			foreach (var item in itemsToRender)
			{
				...
				builder.OpenComponent(1, typeof(EcCheckbox));

				builder.AddAttribute(2, nameof(EcCheckbox.Text), SelectorHelpers.GetText(ItemTextSelector, item));
				builder.AddAttribute(3, nameof(EcCheckbox.Value), Value?.Contains(value) ?? false);
				builder.AddAttribute(4, nameof(EcCheckbox.ValueChanged), EventCallback.Factory.Create<bool>(this, @checked => HandleValueChanged(@checked, item)));
				builder.AddAttribute(5, nameof(EcCheckbox.Enabled), EnabledEffective);

				builder.AddAttribute(6, nameof(EcCheckbox.CssClass), CssClassHelper.Combine(ItemCssClass, ItemCssClassSelector?.Invoke(item)));
				builder.AddAttribute(7, nameof(EcCheckbox.InputCssClass), CssClassHelper.Combine(ItemInputCssClass, ItemInputCssClassSelector?.Invoke(item)));
				builder.AddAttribute(8, nameof(EcCheckbox.TextCssClass), CssClassHelper.Combine(ItemTextCssClass, ItemTextCssClassSelector?.Invoke(item)));

				// We need ValueExpression. Ehm, EcCheckbox needs ValueExpression. Because it is InputBase<T> which needs ValueExpression.
				// We have nothing to give the EcCheckbox. So we make own class with property which we assign to the ValueExpression.
				// Impacts? Unknown. Maybe none.
				builder.AddAttribute(50, nameof(EcCheckbox.ValueExpression), (Expression<Func<bool>>)(() => uglyHack.HackProperty));

				builder.AddAttribute(51, nameof(EcCheckbox.ValidationMessageMode), EnchantedCoder.Blazor.Components.Web.Bootstrap.ValidationMessageMode.None);
				builder.AddAttribute(52, nameof(EcCheckbox.Inline), this.Inline);
				builder.AddAttribute(53, nameof(EcCheckbox.GenerateChip), false);
	...
	/// <summary>
	/// Throws NotSupportedException - giving focus to an input element is not supported on the EcCheckboxList.
	/// </summary>
	public override ValueTask FocusAsync()
	{
		throw new NotSupportedException($"{nameof(FocusAsync)} is not supported on {nameof(EcCheckboxList<TValue, TItem>)}.");
	}
