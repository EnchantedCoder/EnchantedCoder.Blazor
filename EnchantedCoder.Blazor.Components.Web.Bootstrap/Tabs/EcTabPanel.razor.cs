...
/// <summary>
/// Container for <see cref="EcTab"/>s for easier implementation of tabbed UI.<br/>
/// Encapsulates <see cref="EcNav"/> (<see cref="NavVariant.Tabs"/> variant as default) and <see cref="EcNavLink"/>s so you don't have to bother with them explicitly.<br />
/// Full documentation and demos: <see href="https://EnchantedCoder.blazor.eu/components/EcTabPanel">https://EnchantedCoder.blazor.eu/components/EcTabPanel</see>
/// </summary>
public partial class EcTabPanel : ComponentBase, IAsyncDisposable
{
	/// <summary>
	...
	private EcTab previousActiveTab;
	private List<EcTab> tabsList;
	private CollectionRegistration<EcTab> tabsListRegistration;
	private bool isDisposed = false;

	public EcTabPanel()
	{
		tabsList = new List<EcTab>();
		tabsListRegistration = new CollectionRegistration<EcTab>(tabsList,
	...
	{
		EcTab activeTab = tabsList.FirstOrDefault(tab => IsActive(tab));
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		...
		if (firstRender)
		{
			...
			{
				EcTab tabToActivate = tabsList.FirstOrDefault(tab => CascadeEnabledComponent.EnabledEffective(tab) && tab.Visible);
	...
	/// </summary>
	protected async Task HandleTabClick(EcTab tab)
	{
		await SetActiveTabIdAsync(tab.Id);
	}

	private bool IsActive(EcTab tab)
	{
