...
public class ComponentApiDocModelBuilder : IComponentApiDocModelBuilder
{
	...
	private DocXmlReader LoadDocXmlReaderBasedOnNamespace(string typeNamespace)
	{
		...
		{
			return docXmlProvider.GetDocXmlReaderFor("EnchantedCoder.Blazor.Components.Web.Bootstrap.xml");
		}
		else if (typeNamespace.Contains("EnchantedCoder.Blazor.GoogleTagManager"))
		{
			return docXmlProvider.GetDocXmlReaderFor("EnchantedCoder.Blazor.GoogleTagManager.xml");
		}
		else if (typeNamespace.Contains("Bootstrap"))
		{
			return docXmlProvider.GetDocXmlReaderFor("EnchantedCoder.Blazor.Components.Web.Bootstrap.xml");
		}
		else if (typeNamespace == "EnchantedCoder.Blazor.Components.Web")
		{
			return docXmlProvider.GetDocXmlReaderFor("EnchantedCoder.Blazor.Components.Web.xml");
		}
		else
		{
			return docXmlProvider.GetDocXmlReaderFor("EnchantedCoder.Blazor.Components.Web.Bootstrap.xml");
	...
	private void MapProperties(DocXmlReader reader, ComponentApiDocModel model)
	{
		...
		{
			Type nongenericType = Type.GetType($"EnchantedCoder.Blazor.Components.Web.Bootstrap.{ApiRenderer.RemoveSpecialCharacters(model.Type.Name)}, EnchantedCoder.Blazor.Components.Web.Bootstrap");
