...
/// </summary>
public class EcInputFileCore : InputFile, IAsyncDisposable
{
	/// <summary>
	/// Application-wide defaults for the <see cref="EcInputFileCore"/>.
	/// </summary>
	public static InputFileCoreSettings Defaults { get; }

	static EcInputFileCore()
	{
	...
	protected long MaxFileSizeEffective => this.MaxFileSize ?? this.GetSettings()?.MaxFileSize ?? GetDefaults().MaxFileSize ?? throw new InvalidOperationException(nameof(MaxFileSize) + " default for " + nameof(EcInputFileCore) + " has to be set.");

	/// <summary>
	/// Maximum number of concurrent uploads. Default is <c>6</c>.
	/// </summary>
	[Parameter] public int? MaxParallelUploads { get; set; }
	protected int MaxParallelUploadsEffective => this.MaxParallelUploads ?? this.GetSettings()?.MaxParallelUploads ?? this.GetDefaults().MaxParallelUploads ?? throw new InvalidOperationException(nameof(MaxParallelUploads) + " default for " + nameof(EcInputFileCore) + " has to be set.");
	...
	private DotNetObjectReference<EcInputFileCore> dotnetObjectReference;
	...
	/// <remarks>
	/// We do not want to make the EnchantedCoder.Blazor library dependant on WebAssembly libraries (IAccessTokenProvider and such). Therefor the accessToken here...
	/// </remarks>
	...
	{
		jsModule ??= await JSRuntime.ImportEnchantedCoderBlazorWebModuleAsync(nameof(EcInputFileCore));
	}
	...
	/// </summary>
	[JSInvokable("EcInputFileCore_HandleUploadProgress")]
	...
	/// </summary>
	[JSInvokable("EcInputFileCore_HandleFileUploaded")]
	...
	/// Receive upload finished notification from underlying javascript.
	/// </summary>
	[JSInvokable("EcInputFileCore_HandleUploadCompleted")]
